...
- **Listing `Card` titles are `h2` at Block-h3 size (`text-2xl font-bold`).** Listing and related `CardGrid`s are three columns (PA4, N3, AB9, PC2, N13, AB11, HM4). AB7 people groups and N14 rosters stay four — they use `CardGrid` only, not `Card`. Related-strip headings stay at the same rank as the cards they introduce; no `titleAs` fork. [167, 189]
