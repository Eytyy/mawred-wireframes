...
- **The page-header band (breadcrumb + H1) is scaffold, not a block.** It is composed in the page shell, not in layout. Home is the only page that omits it — it opens on HM1. Every other page mounts `<PageHeaderBand />`. On the editorial post and the publication detail the band is crumb only; the H1 lives in the reading column from `lg` (on the editorial post, N7 wraps it: category above, date below). Below `lg` the publication title follows the cover (compact portrait beside it from `md`). The editorial post also closes with a page-level ← prev / next → pair at the bottom of its shell, not a block. `suppressPageHead` is gone. Blocks do not render crumb or page title. [33, 137, 147, 152, 199, 201, 202, 205, 206, 208, 209]
