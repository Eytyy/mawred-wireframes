...
- **Listing `Card` titles are `h2` at Block-h3 size (`text-2xl font-bold`).** Listing and related `CardGrid`s are three columns (AB9, AB11, HM4). PA4 and PC2 are `LibraryRow`s (compact Cover · author · title · languages). N3 and N13 are the same stack (compact Cover · date · title · category), not `DirRow`. AB7 people groups and N14 rosters stay four — they use `CardGrid` only, not `Card`. Related-strip headings stay at the same rank as the cards they introduce; no `titleAs` fork. [167, 189, 191, 192]
