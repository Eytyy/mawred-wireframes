...
- **The page-header band (breadcrumb + H1) is scaffold, not a block.** It is composed in the page shell, not in layout. Home is the only page that omits it — it opens on HM1. Every other page mounts `<PageHeaderBand />`. On both news posts, the publication detail and the network record the band is crumb only; the H1 lives in the reading column from `lg` (on both news posts, N7 wraps it: category above, date below; on the network record, MN6 renders it). Below `lg` the publication title follows the cover (compact portrait beside it from `md`); the network record title precedes the cover, then sits beside a compact portrait from `md`. Both news posts close with a page-level ← prev / next → pair at the bottom of the shell, not a block. `suppressPageHead` is gone. Blocks do not render crumb. [33, 137, 147, 152, 199, 201, 202, 205, 206, 208, 209, 210, 217]
