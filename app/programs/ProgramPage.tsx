...
function renderBlock(block: ProgramBlockConfig): ReactNode {
  switch (block.type) {
    ...
    case 'C5':
      return <C5FormsOfSupport key="C5" records={block.records} heading={block.heading} />;
    case 'C6':
      return <C6HighlightedNote key={block.label} label={block.label} text={block.text} />;
    case 'C8':
    ...
    case 'C10':
      return <C10ContractTerms key="C10" lines={block.lines} items={block.items} />;
    case 'S1':
...
export function ProgramPage({ config }: { config: ProgramConfig }) {
  ...
  const navItems = [
    ...
    ...(showPast && past ? [{ id: 'past', label: past.label ?? 'Past beneficiaries' }] : []),
    { id: 'faqs', label: 'FAQs' },
  ];
  ...
  return (
    <>
      ...
      <section id="about" className="scroll-mt-(--band-h) border-b border-black pb-8">
        <PageWidth>
          <h2 className="sr-only">About</h2>
          <C1Overview text={config.overview?.text} items={config.overview?.items} />
          {config.figs ? (
            <C2ImpactFigures labels={config.figs} />
          ) : (
            <Hint>No impact figures on this programme. By design, not a gap.</Hint>
          )}
          {config.hero ? <S4HeroSummary cells={config.hero} /> : null}
          {overviewBody.map((block) => renderBlock(block))}
      ...
      {showPast && past ? (
        <section id="past" className="scroll-mt-(--band-h) border-b border-black py-8">
          <PageWidth>
      ...
      <section id="faqs" className="scroll-mt-(--band-h) pt-8">
        <PageWidth>
